...
describe('Email Service', () => {
  ...
  describe('Email Sending', () => {
    ...
    test('should throw an error if recipient is missing', async () => {
      ...
      // Act & Assert
      await expect(emailService.sendEmail(emailOptions)).rejects.toThrow('Recipient email is required');
    });
    ...
    test('should throw an error if subject is missing', async () => {
      ...
      // Act & Assert
      await expect(emailService.sendEmail(emailOptions)).rejects.toThrow('Email subject is required');
    });
  ...
  describe('Specialized Email Functions', () => {
    ...
    test('should throw error if verification email is missing required fields', async () => {
      ...
      // Act & Assert
      await expect(emailService.sendVerificationEmail(options)).rejects.toThrow('First name is required');
    });
    ...
    test('should throw error if password reset email is missing required fields', async () => {
      ...
      // Act & Assert
      await expect(emailService.sendPasswordResetEmail(options)).rejects.toThrow('Reset URL is required');
    });
    ...
    test('should throw error if welcome email is missing required fields', async () => {
      ...
      // Act & Assert
      await expect(emailService.sendWelcomeEmail(options)).rejects.toThrow('Dashboard URL is required');
    });
  });
  ...
  describe('Accessibility', () => {
    test('default template should have no accessibility violations', () => {
      // Arrange
      const template = '<html lang="en"><head><meta name="viewport" content="width=device-width, initial-scale=1.0"><title>Test Email</title></head><body><a href="#main" class="skip-link">Skip to content</a><h1>Test Email</h1><p>This is a test email</p><img src="logo.png" alt="Company Logo"><a href="https://example.com">Visit our website</a></body></html>';
    ...
    test('verification email template should be accessible', () => {
      // Arrange
      const template = '<html lang="en"><head><title>Verify Your Email</title></head><body><h1>Verify Your Email</h1><p>Hello John,</p><p>Please click the link below to verify your email:</p><a href="https://example.com/verify/token123">Verify Email</a></body></html>';
