...
const ALLOWED_FILE_EXTENSIONS = (
  process.env.ALLOWED_FILE_EXTENSIONS || '.jpg,.jpeg,.png,.gif,.pdf,.doc,.docx'
).split(',');
...
const storeFile = async (file, isPrivate = false) => {
  ...
  if (process.env.NODE_ENV === 'test') {
    ...
    )}-${uniqueId}${path.extname(sanitizedName)}`;
...
const fileFilter = (req, file, cb) => {
  if (ALLOWED_FILE_TYPES.includes(file.mimetype)) {
    ...
    cb(
      new ApiError(400, `File type not allowed. Allowed types: ${ALLOWED_FILE_TYPES.join(', ')}`),
      false
    );
  }
};
...
const imageFilter = (req, file, cb) => {
  if (ALLOWED_IMAGE_TYPES.includes(file.mimetype)) {
    ...
    cb(
      new ApiError(
        400,
        `File type not allowed. Allowed image types: ${ALLOWED_IMAGE_TYPES.join(', ')}`
      ),
      false
    );
  }
...
const documentFilter = (req, file, cb) => {
  if (ALLOWED_DOCUMENT_TYPES.includes(file.mimetype)) {
    ...
    cb(
      new ApiError(
        400,
        `File type not allowed. Allowed document types: ${ALLOWED_DOCUMENT_TYPES.join(', ')}`
      ),
      false
    );
  }
